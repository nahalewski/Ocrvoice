...
function renderExtracted(){
  if(!extracted.length){$("extractedAccounts").innerHTML="";return}
  const reviewed=extracted.filter(item=>item.decision).length;
  if(reviewCursor>=extracted.length){const letters=extracted.filter(item=>item.decision==="unknown"||item.decision==="fraud").length;$("extractedAccounts").innerHTML=`<div class="form-card wizard-done"><span class="step">DONE</span><h3>Review complete</h3><p>${reviewed} accounts reviewed. ${letters} dispute letter${letters===1?"":"s"} ready.</p><button id="goLetters" class="primary">Open my letters</button><button id="reviewAgain">Review again</button></div>`;$("goLetters").onclick=()=>showTab("letters");$("reviewAgain").onclick=()=>{reviewCursor=0;renderExtracted()};return}
  const item=extracted[reviewCursor],summary=item.reported.split("\n").filter(line=>/^(Account Type|Balance|Pay Status|Date Opened|Estimated month)/i.test(line)).slice(0,5).join("\n");
  $("extractedAccounts").innerHTML=`<div class="form-card wizard"><div class="progress"><span>STEP 2 · ACCOUNT ${reviewCursor+1} OF ${extracted.length}</span><span>${reviewed} reviewed</span></div><div class="progress-track"><i style="width:${Math.round(reviewed/extracted.length*100)}%"></i></div><div class="mark-card"><h2>${esc(item.title)}</h2><pre class="account-summary">${esc(summary||item.reported.split("\n").slice(0,5).join("\n"))}</pre><details><summary>Show all report details</summary><pre>${esc(item.reported)}</pre></details><p class="choice-prompt">Which statement is true?</p><div class="quick-actions"><button data-quick="unknown" data-extracted="${reviewCursor}" class="primary">I don't recognize it</button><button data-quick="fraud" data-extracted="${reviewCursor}">It is identity theft / fraud</button><button data-quick="skip" data-extracted="${reviewCursor}">It is mine — skip</button></div></div><div class="wizard-nav"><button id="previousAccount" ${reviewCursor===0?"disabled":""}>Back</button></div></div>`;
  ...
  $("previousAccount").onclick=()=>{reviewCursor=Math.max(0,reviewCursor-1);renderExtracted()};
}
function applyQuickDecision(index,decision){
  ...
  if(decision==="skip"){if(existing>=0)state.marks.splice(existing,1);save();reviewCursor=index+1;renderExtracted();renderAll();return}
  ...
  if(existing>=0)state.marks[existing]=mark;else state.marks.push(mark);save();reviewCursor=index+1;renderExtracted();renderAll();
}
$("scanReport").onclick=async()=>{
  const file=$("reportFile").files[0];if(!file)return alert("Choose a PDF report first.");
  $("scanStatus").textContent="Reading the PDF on this device…";$("scanReport").disabled=true;extracted=[];reviewCursor=0;renderExtracted();
...
function showTab(id){document.querySelectorAll("nav [data-tab],.tab").forEach(node=>node.classList.remove("active"));document.querySelector(`nav [data-tab="${id}"]`)?.classList.add("active");$(id).classList.add("active");window.scrollTo({top:0,behavior:"smooth"})}
document.querySelectorAll("nav [data-tab]").forEach(button=>button.onclick=()=>showTab(button.dataset.tab));
$("toggleAdvanced").onclick=()=>{const open=$("toggleAdvanced").getAttribute("aria-expanded")==="true";$("toggleAdvanced").setAttribute("aria-expanded",String(!open));document.querySelectorAll(".advanced-nav").forEach(button=>button.classList.toggle("shown",!open))};
...
function renderLetters(){
  ...
  $("letterList").innerHTML=eligible.length?`<div class="form-card letters-ready"><span class="step">STEP 3</span><h3>${eligible.length} letter${eligible.length===1?"":"s"} ready</h3><p>Download everything at once, then follow the short instructions for each company.</p><button id="downloadAllLetters" class="primary">Download all letters</button></div>${eligible.map(({mark,index})=>{const channel=disputeChannels[mark.bureau]||{name:mark.bureau,mode:"assisted",url:"#"},steps=filingSteps(channel,mark);return `<article class="mark-card letter-card"><span class="badge yes">${esc(mark.disputeKind==="fraud"?"Fraud":"Not recognized")}</span><h3>${esc(mark.creditor)}</h3><ol>${steps.map(step=>`<li>${esc(step)}</li>`).join("")}</ol>${mark.disputeKind==="fraud"?'<p><a href="https://www.identitytheft.gov/" target="_blank" rel="noopener noreferrer">Start at IdentityTheft.gov</a></p>':""}<p><a href="${esc(channel.url)}" target="_blank" rel="noopener noreferrer">Open ${esc(channel.name)} instructions</a></p><button data-letter-download="${index}" class="primary">Download this letter</button> <button data-letter-copy="${index}">Copy</button><details><summary>Edit or preview letter</summary><label>Letter<textarea id="letter-${index}" rows="18">${esc(letterText(mark))}</textarea></label></details></article>`}).join("")}`:"<div class='form-card'><p class='subtle'>No letters are ready. Upload a report and choose “I don't recognize this” or “Identity theft / fraud” for the applicable account.</p></div>";
  const all=$("downloadAllLetters");if(all)all.onclick=()=>downloadText("all-dispute-letters.txt",eligible.map(({mark})=>letterText(mark)).join("\n\n==============================\n\n"));
